...
def trainHumanoid_fast(it: int,t: float, g: float, a:float):
    ...
    def _func(i: int, t: float, g: float, a: float, it: int):
        ...
        pre = f"./savedModels/temp/HumanoidStandup-v4_tau{model.tau}gamma{model.gamma}alpha{model.learning_rate}_{i}"
        model = SAC.load(pre, vec_env_train)    
        model.learn(50_000, progress_bar=True)
        pre = pre = f"./savedModels/temp/HumanoidStandup-v4_tau{model.tau}gamma{model.gamma}alpha{model.learning_rate}_{i+1}"
        model.save(pre)
        if (i % 19 == 0):
            model.save(f"./savedModels/second/HumanoidStandup-v4_tau{model.tau}gamma{model.gamma}alpha{model.learning_rate}_{it+1}M")
    ...
    model = SAC.load(f"./savedModels/second/HumanoidStandup-v4_tau{model.tau}gamma{model.gamma}alpha{model.learning_rate}_{it}M", vec_env_train)    
    model.learn(50_000, progress_bar=True)
    pre = f"./savedModels/temp/HumanoidStandup-v4_tau{model.tau}gamma{model.gamma}alpha{model.learning_rate}_{1}"
    model.save(pre)
    print("saved model:\t", pre)
    for i in range(1, 20):
        _func(i, t, g, a, it)
